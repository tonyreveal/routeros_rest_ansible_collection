...
---
module: bgp_info
short_description: Manage or gather RouterOS bgp info information
seealso:
...
attributes:
  check_mode:
    ...
    support: full
description:
  - Reads BGP template records from RouterOS.
...
EXAMPLES = r"""
---
- name: Gather BGP template
  hosts: localhost
  gather_facts: false
  tasks:
    - name: Read BGP template
      mikrotik.routeros_rest.bgp_info:
        host: https://192.0.2.1
        username: "{{ vault_routeros_username }}"
        password: "{{ vault_routeros_password }}"
      register: routeros_bgp_info

    - name: Show returned data
      ansible.builtin.debug:
        var: routeros_bgp_info.templates
...
"""
...
def main() -> None:
    ...
    run_info(module, "routing/bgp/template", "templates", "name")
