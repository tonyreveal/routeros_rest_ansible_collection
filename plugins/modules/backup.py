...
---
module: backup
short_description: Manage or gather RouterOS backup information
seealso:
...
attributes:
  check_mode:
    ...
    support: full
description:
  - Creates a native RouterOS binary backup through the REST API.
  - The filename is built as prefix-YYYYMMDD-HHMM.backup using UTC time.
...
options:
  ...
  filename_prefix:
    description:
      - Safe prefix for the generated binary backup filename.
      - The module appends -YYYYMMDD-HHMM.backup.
    type: str
    default: ansible-backup
notes:
  ...
  - The backup file remains on the RouterOS device and must be retrieved separately.
  - Native binary backups can contain credentials and sensitive configuration data.
requirements:
  - Python 3
...
EXAMPLES = r"""
---
- name: Create a RouterOS binary backup
      mikrotik.routeros.backup:
    host: https://192.0.2.1
    username: "{{ vault_routeros_username }}"
    password: "{{ vault_routeros_password }}"
    filename_prefix: ansible-backup
  register: backup_result

- name: Display generated backup filename
  ansible.builtin.debug:
    var: backup_result.filename
...
"""
...
RETURN = r"""
filename:
  description: Generated binary backup filename, including the .backup extension.
  returned: always
  type: str
  sample: ansible-backup-20260825-1345.backup
result:
...
def main() -> None:
    ...
    filename = f"{params['filename_prefix']}-{datetime.now(timezone.utc):%Y%m%d-%H%M}.backup"
    ...
    try:
        result = client.post("system/backup/save", {"name": filename})
