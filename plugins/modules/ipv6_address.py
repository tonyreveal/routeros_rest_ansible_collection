...
DOCUMENTATION = r"""---
module: ipv6_address
short_description: Manage or gather RouterOS ipv6 address information
description:
...
def main():
    ...
    p = m.params
    d = {"address": p["address"], "interface": p["interface"], "disabled": p["disabled"]}
    try:
        reconcile(
            m,
            RouterOSRestClient(p["host"], p["username"], p["password"], p["timeout"], p["validate_certs"]),
            "ipv6/address",
            {"address": p["address"], "interface": p["interface"]},
            d,
            p["state"],
            "address",
        )
