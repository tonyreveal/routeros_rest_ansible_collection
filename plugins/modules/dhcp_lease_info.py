...
DOCUMENTATION = r"""
---
module: dhcp_lease_info
short_description: Manage or gather RouterOS dhcp lease info information
seealso:
...
attributes:
  check_mode:
    ...
    support: full
description:
  - Reads DHCP lease records from RouterOS.
...
EXAMPLES = r"""
---
- name: Gather DHCP lease
  hosts: localhost
  gather_facts: false
  tasks:
    - name: Read DHCP lease
      mikrotik.routeros_rest.dhcp_lease_info:
        host: https://192.0.2.1
        username: "{{ vault_routeros_username }}"
        password: "{{ vault_routeros_password }}"
      register: routeros_dhcp_lease_info

    - name: Show returned data
      ansible.builtin.debug:
        var: routeros_dhcp_lease_info.leases
...
"""
...
def main() -> None:
    ...
    run_info(module, "ip/dhcp-server/lease", "leases", "address")
