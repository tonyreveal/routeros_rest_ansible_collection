...
---
module: ntp_info
short_description: Manage or gather RouterOS ntp info information
seealso:
...
attributes:
  check_mode:
    ...
    support: full
description:
  - Reads NTP client records from RouterOS.
...
EXAMPLES = r"""
---
- name: Gather NTP client
  hosts: localhost
  gather_facts: false
  tasks:
    - name: Read NTP client
      mikrotik.routeros_rest.ntp_info:
        host: https://192.0.2.1
        username: "{{ vault_routeros_username }}"
        password: "{{ vault_routeros_password }}"
      register: routeros_ntp_info

    - name: Show returned data
      ansible.builtin.debug:
        var: routeros_ntp_info.ntp
...
"""
...
def main() -> None:
    ...
    run_info(module, "system/ntp/client", "ntp", None)
