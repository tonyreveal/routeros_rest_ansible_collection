...
DOCUMENTATION = r"""
---
module: firewall_filter_info
short_description: Manage or gather RouterOS firewall filter info information
seealso:
...
attributes:
  check_mode:
    ...
    support: full
description:
  - Reads IPv4 firewall filter records from RouterOS.
...
EXAMPLES = r"""
---
- name: Gather IPv4 firewall filter
  hosts: localhost
  gather_facts: false
  tasks:
    - name: Read IPv4 firewall filter
      mikrotik.routeros_rest.firewall_filter_info:
        host: https://192.0.2.1
        username: "{{ vault_routeros_username }}"
        password: "{{ vault_routeros_password }}"
      register: routeros_firewall_filter_info

    - name: Show returned data
      ansible.builtin.debug:
        var: routeros_firewall_filter_info.rules
...
"""
...
def main() -> None:
    ...
    run_info(module, "ip/firewall/filter", "rules", "comment")
