...
DOCUMENTATION = r'''
---
module: container_start
short_description: Start a RouterOS container
description:
  - Ensures a named RouterOS container is running through the REST API.
  - Repeated runs do not start an already running container.
version_added: '1.0.0'
author:
...
options:
  ...
  name:
    ...
    required: true
  state:
    description: Present starts the container; absent leaves it stopped.
    type: str
    choices:
      - present
      - absent
...
attributes:
  check_mode:
    description: Reports whether a start operation would occur.
    support: full
'''
EXAMPLES = r'''
---
- name: Start a container
  mikrotik.routeros_rest.container_start:
...
def main() -> None:
    ...
    try:
        ...
        if p["state"] == "absent" or running: module.exit_json(changed=False, result=current or {})
        if module.check_mode: module.exit_json(changed=True)
        module.exit_json(changed=True, result=client.post("container/start", {"name": p["name"]}))
