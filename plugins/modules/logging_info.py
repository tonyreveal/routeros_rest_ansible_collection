...
---
module: logging_info
short_description: Manage or gather RouterOS logging info information
seealso:
...
attributes:
  check_mode:
    ...
    support: full
description:
  - Reads logging rule records from RouterOS.
...
EXAMPLES = r"""
---
- name: Gather logging rule
  hosts: localhost
  gather_facts: false
  tasks:
    - name: Read logging rule
      mikrotik.routeros_rest.logging_info:
        host: https://192.0.2.1
        username: "{{ vault_routeros_username }}"
        password: "{{ vault_routeros_password }}"
      register: routeros_logging_info

    - name: Show returned data
      ansible.builtin.debug:
        var: routeros_logging_info.rules
...
"""
...
def main() -> None:
    ...
    run_info(module, "system/logging", "rules", "topics")
