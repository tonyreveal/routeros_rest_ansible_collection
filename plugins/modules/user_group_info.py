...
short_description: Gather RouterOS user group information
description:
  - Reads RouterOS user groups through the REST API.
...
options:
  ...
  password:
    ...
    no_log: true
  name:
    description: Optional user group name filter.
    type: str
  object_id:
...
- name: Read user groups
  mikrotik.routeros_rest.user_group_info:
...
RETURN = r'''
groups:
  description: RouterOS user group records.
  returned: always
  type: list
  elements: dict
'''
...
def main() -> None:
    ...
    run_info(module, "user/group", "groups", "name")
