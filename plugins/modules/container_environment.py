#!/usr/bin/python
# Copyright: Tony Reveal
# GNU General Public License v3.0 or later (see LICENSE or https://www.gnu.org/licenses/gpl-3.0.html)

from ansible.module_utils.basic import AnsibleModule
from ansible_collections.mikrotik.routeros_rest.plugins.module_utils.config_resource import run_config

DOCUMENTATION = r'''
---
module: container_environment
short_description: Manage RouterOS container environment entries
description:
  - Creates, updates, or removes a RouterOS container environment entry through the REST API.
version_added: '1.0.0'
author:
  - Tony Reveal (https://github.com/tonyreveal)
options:
  host:
    description: RouterOS REST base URL.
    type: str
    required: true
  username:
    description: RouterOS REST username.
    type: str
    required: true
  password:
    description: RouterOS REST password.
    type: str
    required: true
    no_log: true
  name:
    description: Environment-list or entry name.
    type: str
    required: true
  settings:
    description: Container environment properties.
    type: dict
    required: true
    suboptions:
      key:
        description: Environment variable name.
        type: str
        required: true
      value:
        description: Environment variable value.
        type: str
        no_log: true
      comment:
        description: Optional comment.
        type: str
  state:
    description: Desired environment entry state.
    type: str
    choices:
      - present
      - absent
    default: present
  validate_certs:
    description: Validate the RouterOS TLS certificate.
    type: bool
    default: true
  timeout:
    description: HTTP timeout in seconds.
    type: int
    default: 30
requirements:
  - Python 3.
  - RouterOS 7.x with REST API and container support enabled.
  - Ansible 2.16 or newer.
attributes:
  check_mode:
    description: Supports check mode without changing the device.
    support: full
'''
EXAMPLES = r'''
---
- name: Configure a container environment variable
  mikrotik.routeros_rest.container_environment:
    host: https://router.example.test
    username: admin
    password: secret
    name: dns-env
    settings:
      key: DNS_MODE
      value: recursive
'''
RETURN = r'''
environment:
  description: Container environment entry returned after a change.
  returned: on change
  type: dict
'''


def main() -> None:
    module = AnsibleModule(argument_spec={"host": {"type": "str", "required": True}, "username": {"type": "str", "required": True}, "password": {"type": "str", "required": True, "no_log": True}, "name": {"type": "str", "required": True}, "settings": {"type": "dict", "required": True}, "state": {"type": "str", "choices": ["present", "absent"], "default": "present"}, "validate_certs": {"type": "bool", "default": True}, "timeout": {"type": "int", "default": 30}}, supports_check_mode=True)
    p = module.params
    run_config(module, "container/envs", {"name": p["name"]}, {"name": p["name"], **p["settings"]})
if __name__ == "__main__": main()
