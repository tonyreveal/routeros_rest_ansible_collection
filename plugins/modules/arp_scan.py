...
---
module: arp_scan
short_description: Run a RouterOS ARP scan
description:
  - Runs an ARP discovery operation through the RouterOS REST API.
...
options:
  ...
  password:
    ...
    no_log: true
  interface:
    description: Interface on which to discover neighbors.
    type: str
    required: true
  duration:
...
EXAMPLES = r'''
---
- name: Discover ARP neighbors
  mikrotik.routeros_rest.arp_scan:
    host: https://router.example.test
    username: admin
    password: secret
    interface: bridge-lan
  register: arp_results
'''
RETURN = r'''
result:
  description: RouterOS ARP scan response.
  returned: success
  type: raw
'''
...
def main() -> None:
    ...
    run_tool(module, "ip/arp", {"interface": p["interface"], "duration": str(p["duration"])})
