...
---
module: route_info
short_description: Manage or gather RouterOS route info information
seealso:
...
attributes:
  check_mode:
    ...
    support: full
description:
  - Reads IPv4 route records from RouterOS.
...
EXAMPLES = r"""
---
- name: Gather IPv4 route
  hosts: localhost
  gather_facts: false
  tasks:
    - name: Read IPv4 route
      mikrotik.routeros_rest.route_info:
        host: https://192.0.2.1
        username: "{{ vault_routeros_username }}"
        password: "{{ vault_routeros_password }}"
      register: routeros_route_info

    - name: Show returned data
      ansible.builtin.debug:
        var: routeros_route_info.routes
...
"""
...
def main() -> None:
    ...
    run_info(module, "ip/route", "routes", "dst-address")
