...
DOCUMENTATION = r"""
---
module: firewall_nat_info
short_description: Manage or gather RouterOS firewall nat info information
seealso:
...
attributes:
  check_mode:
    ...
    support: full
description:
  - Reads IPv4 NAT records from RouterOS.
...
EXAMPLES = r"""
---
- name: Gather IPv4 NAT
  hosts: localhost
  gather_facts: false
  tasks:
    - name: Read IPv4 NAT
      mikrotik.routeros_rest.firewall_nat_info:
        host: https://192.0.2.1
        username: "{{ vault_routeros_username }}"
        password: "{{ vault_routeros_password }}"
      register: routeros_firewall_nat_info

    - name: Show returned data
      ansible.builtin.debug:
        var: routeros_firewall_nat_info.rules
...
"""
...
def main() -> None:
    ...
    run_info(module, "ip/firewall/nat", "rules", "comment")
