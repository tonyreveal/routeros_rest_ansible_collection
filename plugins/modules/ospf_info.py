...
---
module: ospf_info
short_description: Manage or gather RouterOS ospf info information
seealso:
...
attributes:
  check_mode:
    ...
    support: full
description:
  - Reads OSPF instance records from RouterOS.
...
EXAMPLES = r"""
---
- name: Gather OSPF instance
  hosts: localhost
  gather_facts: false
  tasks:
    - name: Read OSPF instance
      mikrotik.routeros_rest.ospf_info:
        host: https://192.0.2.1
        username: "{{ vault_routeros_username }}"
        password: "{{ vault_routeros_password }}"
      register: routeros_ospf_info

    - name: Show returned data
      ansible.builtin.debug:
        var: routeros_ospf_info.instances
...
"""
...
def main() -> None:
    ...
    run_info(module, "routing/ospf/instance", "instances", "name")
