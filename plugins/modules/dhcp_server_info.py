...
short_description: Manage or gather RouterOS dhcp server info information
seealso:
...
attributes:
  check_mode:
    ...
    support: full
description:
  - Reads DHCP server records from RouterOS.
...
EXAMPLES = r"""
---
- name: Gather DHCP server
  hosts: localhost
  gather_facts: false
  tasks:
    - name: Read DHCP server
      mikrotik.routeros_rest.dhcp_server_info:
        host: https://192.0.2.1
        username: "{{ vault_routeros_username }}"
        password: "{{ vault_routeros_password }}"
      register: routeros_dhcp_server_info

    - name: Show returned data
      ansible.builtin.debug:
        var: routeros_dhcp_server_info.servers
...
"""
...
def main() -> None:
    ...
    run_info(module, "ip/dhcp-server", "servers", "name")
