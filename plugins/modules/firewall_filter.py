...
short_description: Manage or gather RouterOS firewall filter information
description:
...
def main(path="ip/firewall/filter"):
    m = AnsibleModule(
        argument_spec={
    ...
    p = m.params
    d = {**p["rule"], "comment": p["name"]} if "comment" not in p["rule"] else p["rule"]
    d["comment"] = p["name"]
    try:
        reconcile(
            m,
