...
---
module: sstp_server
short_description: Manage RouterOS SSTP server settings through the REST API
description:
  - Reconciles the RouterOS SSTP server singleton.
  - The settings dictionary contains RouterOS SSTP server properties.
version_added: '1.0.0'
author:
...
options:
  ...
  password:
    ...
    no_log: true
  settings:
    description: RouterOS SSTP server properties.
    type: dict
    required: true
    suboptions:
      enabled:
        description: Whether the SSTP server is enabled.
        type: bool
      default_profile:
...
def main() -> None:
    ...
    run_config(module, "interface/sstp-server/server", {}, settings)
