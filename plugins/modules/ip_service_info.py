...
DOCUMENTATION = r"""
---
module: ip_service_info
short_description: Manage or gather RouterOS ip service info information
seealso:
...
attributes:
  check_mode:
    ...
    support: full
description:
  - Reads IP service records from RouterOS.
...
EXAMPLES = r"""
---
- name: Gather IP service
  hosts: localhost
  gather_facts: false
  tasks:
    - name: Read IP service
      mikrotik.routeros_rest.ip_service_info:
        host: https://192.0.2.1
        username: "{{ vault_routeros_username }}"
        password: "{{ vault_routeros_password }}"
      register: routeros_ip_service_info

    - name: Show returned data
      ansible.builtin.debug:
        var: routeros_ip_service_info.services
...
"""
...
def main() -> None:
    ...
    run_info(module, "ip/service", "services", "name")
